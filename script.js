    ...
    if (tasks[i].isChecked) {
      ...
      if (activeFilter != 2) {
        html += `<div class="task-card" id="${tasks[i].id}">
                    ...
                    <button onclick="editTask(${tasks[i].id})"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18">
...
function editTask(id) {
  ...
  for (let i = 0; i < tasks.length; i++) {
    if (id == tasks[i].id) {
      temp = tasks[i].description;
      tasks.splice(i, 1);
    }
  }
  setCount();
  listTasks();
  document.getElementById("new-task").value = temp;
}
...
function createNewTask(e) {
  ...
  if(e == -1 ) {
      let desc = document.getElementById('new-task').value;
      let obj = { description: desc, isChecked: false, id: uniqueId() };
      tasks.push(obj);
      listTasks();
  }
}
...
function updateTitle() {
  document.getElementById("title").innerText = `To Do App (${tasks.length})`;
}
